...
class CIFAR10Dataset(Dataset):
    def __init__(
        ...
        self.X = np.concatenate([batch[b"data"] for batch in batches]).astype(np.float32)
        # 原始格式是 (N, 3072)，重排为 (N, C, H, W)
        self.X = np.reshape(self.X, (self.X.shape[0], 3, 32, 32)) / np.float32(255.0)
