...
class RNNCell(Module):
    ...
    def forward(self, X, h=None):
        ...
        if h is None:
            h = init.zeros(
                batch_size, self.hidden_size, device=self.device, dtype=self.dtype
            )
        if self.bias:
...
class LSTMCell(Module):
    ...
    def forward(self, X, h=None):
        ...
        if self.bias:
            ...
            # 拼接后的门控张量形状为 (batch, 4 * hidden_size)
            gates_all = X @ self.W_ih + h0 @ self.W_hh
        # 按列拆分为若干个长度为 1 的小张量
        gates_all_split = tuple(ops.split(gates_all, axis=1))
        gates = []
        for i in range(4):
            # 将拼接的 4 组门控向量还原为 [i,f,g,o]
            gates.append(
                ops.stack(
                    gates_all_split[i * self.hidden_size : (i + 1) * self.hidden_size],
                    axis=1,
                )
            )
        i, f, g, o = gates
...
class LSTM(Module):
    def __init__(
        ...
        self.lstm_cells = [
            ...
            LSTMCell(hidden_size, hidden_size, bias=bias, device=device, dtype=dtype)
            for _ in range(num_layers - 1)
        ]
...
class Embedding(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_one_hot = init.one_hot(
            self.weight.shape[0], x, device=self.device, dtype=self.dtype
        ).reshape((seq_len * bs, self.weight.shape[0]))
