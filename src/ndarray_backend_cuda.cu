...
// strides
__device__ __inline__ size_t md_idx(size_t idx, const CudaVec &shape,
                                    const CudaVec &strides, size_t offset) {
  size_t i = shape.size;
  while (i-- > 0) {
...
__global__ void CompactKernel(const scalar_t *a, scalar_t *out, size_t size,
  ...
  /// BEGIN SOLUTION
  if (gid < size)
    out[gid] = a[md_idx(gid, shape, strides, offset)];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t *a, scalar_t *out,
  ...
  /// BEGIN SOLUTION
  if (gid < size)
    out[md_idx(gid, shape, strides, offset)] = a[gid];
  /// END SOLUTION
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t *out, size_t size,
  ...
  /// BEGIN SOLUTION
  if (gid < size)
    out[md_idx(gid, shape, strides, offset)] = val;
  /// END SOLUTION
}
...
__global__ void EwiseSubKernel(const scalar_t *a, const scalar_t *b,
                               scalar_t *out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size)
    out[gid] = a[gid] - b[gid];
...
__global__ void ScalarSubKernel(const scalar_t *a, scalar_t val, scalar_t *out,
                                size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size)
    out[gid] = a[gid] - val;
...
void EwiseLog(const CudaArray &a, const CudaArray &out) {
  CudaDims dim = CudaOneDim(out.size);
...
void EwiseExp(const CudaArray &a, const CudaArray &out) {
  CudaDims dim = CudaOneDim(out.size);
...
void EwiseTanh(const CudaArray &a, const CudaArray &out) {
  CudaDims dim = CudaOneDim(out.size);
...
__global__ void MatmulKernel(const scalar_t *__restrict__ a,
                             ...
                             scalar_t *__restrict__ out, uint32_t M, uint32_t N,
                             uint32_t P) {
  __shared__ scalar_t a_tile[TILE][TILE], b_tile[TILE][TILE];
  scalar_t s = 0;
  size_t y, x;
  for (size_t k = 0; k < (N + TILE - 1) / TILE; ++k) {
    y = blockIdx.y * TILE + threadIdx.y;
    x = k * TILE + threadIdx.x;
    a_tile[threadIdx.y][threadIdx.x] = (y < M && x < N) ? a[y * N + x] : 0;
    y = k * TILE + threadIdx.y;
    x = blockIdx.x * TILE + threadIdx.x;
    b_tile[threadIdx.y][threadIdx.x] = (y < N && x < P) ? b[y * P + x] : 0;
    __syncthreads();
    for (size_t i = 0; i < TILE; ++i)
      s += a_tile[threadIdx.y][i] * b_tile[i][threadIdx.x];
    __syncthreads();
  }
  y = blockIdx.y * TILE + threadIdx.y;
  x = blockIdx.x * TILE + threadIdx.x;
