...
struct AlignedArray {
  ...
  scalar_t &operator[](size_t idx) { return ptr[idx]; }
  const scalar_t &operator[](size_t idx) const { return ptr[idx]; }
  scalar_t *ptr;
  size_t size;
};

size_t flat_idx(const std::vector<int32_t> &idxs,
                const std::vector<int32_t> &strides, size_t offset = 0) {
  size_t idx = offset;
...
bool incr_idx(std::vector<int32_t> &idxs, const std::vector<int32_t> &shape) {
  size_t i = idxs.size();
  while (i-- > 0) {
    if (++idxs[i] == shape[i])
      idxs[i] = 0;
...
void Compact(const AlignedArray &a, AlignedArray &out,
  ...
  bool succ = true;
  while (succ) {
    out[cnt++] = a[flat_idx(idxs, strides, offset)];
    succ = incr_idx(idxs, shape);
  }
  /// END SOLUTION
}
...
void EwiseSetitem(const AlignedArray &a, AlignedArray &out,
  ...
  bool succ = true;
  while (succ) {
    out[flat_idx(idxs, strides, offset)] = a[cnt++];
    succ = incr_idx(idxs, shape);
  }
  /// END SOLUTION
}
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray &out,
  ...
  bool succ = true;
  while (succ) {
    out[flat_idx(idxs, strides, offset)] = val;
    succ = incr_idx(idxs, shape);
  }
  /// END SOLUTION
}
  ...
  for (size_t i = 0; i < m; i += TILE)
    for (size_t j = 0; j < p; j += TILE)
      for (size_t k = 0; k < n; k += TILE)
        AlignedDot(a.ptr + i * n + k * TILE, b.ptr + k * p + j * TILE,
                   out.ptr + i * p + j * TILE);
  /// END SOLUTION
}
