...
class LanguageModel(nn.Module):
    def __init__(
        ...
        assert seq_model in ["rnn", "lstm", "transformer"], (
            "Unsupported sequence model. Must be rnn, lstm, or transformer."
        )
        ...
        if seq_model == "rnn":
            ...
            linear_in_features = hidden_size
        elif seq_model == "lstm":
            ...
            linear_in_features = embedding_size
        self.linear = nn.Linear(
            linear_in_features, output_size, device=device, dtype=dtype
        )
    ...
    def forward(self, x, h=None):
        ...
        x = self.linear(x.reshape((seq_len * bs, feature_dim)))
        return x, h
